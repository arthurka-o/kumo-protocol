  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    if (!withProxy) {
      ...
      it("approve(): reverts when owner param is address(0)", async () => {
        const txPromise = kusdTokenTester.callInternalApprove(ZERO_ADDRESS, alice, dec(1000, 18), {
          from: bob
        });
        await assertAssert(txPromise);
      });
    ...
    it("decreaseAllowance(): fails trying to decrease more than previously allowed", async () => {
      ...
      assert.equal((await kusdTokenTester.allowance(alice, bob)).toString(), dec(3, 18));
      await assertRevert(
        kusdTokenTester.decreaseAllowance(bob, dec(4, 18), { from: alice }),
    ...
    if (!withProxy) {
      ...
      it("Initializes DOMAIN_SEPARATOR correctly", async () => {
        assert.equal(
          await kusdTokenTester.domainSeparator(),
      ...
      it("permits and emits an Approval event (replay protected)", async () => {
        ...
        await assertRevert(
          kusdTokenTester.permit(approve.owner, approve.spender, approve.value, deadline, v, r, s),
          "KUSD: invalid signature"
        );

        // Check that the zero address fails
        await assertAssert(
          kusdTokenTester.permit(
