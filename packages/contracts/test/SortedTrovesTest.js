    ...
    context("when params are properly set", () => {
      ...
      it("insert(): fails if id is zero", async () => {
        await th.assertRevert(
          sortedTrovesTester.insert(assetAddress1, th.ZERO_ADDRESS, 1, alice, alice),
      ...
      it("insert(): fails if NICR is zero", async () => {
        await th.assertRevert(
          sortedTrovesTester.insert(assetAddress1, alice, 0, alice, alice),
      ...
      it("remove(): fails if id is not in the list", async () => {
        await th.assertRevert(
          sortedTrovesTester.remove(assetAddress1, alice),
      ...
      it("reInsert(): fails if list doesn’t contain the node", async () => {
        await th.assertRevert(
          sortedTrovesTester.reInsert(assetAddress1, alice, 1, alice, alice),
          "SortedTroves: List does not contain the id"
        );
      });
      ...
      it("reInsert(): fails if new NICR is zero", async () => {
        await sortedTrovesTester.insert(assetAddress1, alice, 1, alice, alice);
        assert.isTrue(
          await sortedTroves.contains(assetAddress1, alice),
          "list should contain element"
        );
        await th.assertRevert(
          sortedTrovesTester.reInsert(assetAddress1, alice, 0, alice, alice),
          "SortedTroves: NICR must be positive"
        );
        assert.isTrue(
          await sortedTroves.contains(assetAddress1, alice),
