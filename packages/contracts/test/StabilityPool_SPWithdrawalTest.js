  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100000, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(100000, 18)),
        whale,
        whale,
        { from: whale }
      );
      ...
      // Check depositors' compounded deposit is 6666.66 KUSD and ETH Gain is 33.16 ETH
      ...
      assert.isAtMost(
        th.getDifference(
          (await kusdToken.balanceOf(alice)).toString(),
          toBN("6666666666666666666666")
        ),
        10000
      );
      assert.isAtMost(
        th.getDifference(
          (await kusdToken.balanceOf(bob)).toString(),
          toBN("6666666666666666666666")
        ),
        10000
      );
      assert.isAtMost(
        th.getDifference(
          (await kusdToken.balanceOf(carol)).toString(),
          toBN("6666666666666666666666")
        ),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 3333.33 KUSD and ETH Gain is 66.33 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      // Grab the ETH gain from the emitted event in the tx log
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "3333333333333333333333"),
        10000
      );
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 0 KUSD and ETH Gain is 99.5 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      ...
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(bob)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(carol)).toString(), "0"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing KUSD", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(5000, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(7000, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "6000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing KUSD", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(6000, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "4000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "13333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "20000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "10000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "15000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(1000000, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(1000000, 18)),
        whale,
        whale,
        { from: whale }
      );
      ...
      Defaulter 1: 207000 KUSD & 2160 ETH
      ...
      */
      await borrowerOperations.openTrove(
        ...
        dec(2160, 18),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "207000000000000000000000"),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      await borrowerOperations.openTrove(
        ...
        dec(50, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(5, 21)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      await borrowerOperations.openTrove(
        ...
        dec(500, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "46700000000000000000000"),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3 }
      );
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "901719380174061000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "205592018679686000000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "5906261940140100000000"),
        10000000000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      console.log();
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "1666666666666666666666"),
        100000
      );

      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "0"), 100000);
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 1m ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(1000000, "ether"),
      ...
      */
      ...
      Defaulter 2:  25000 KUSD, 250 ETH
      ...
      */
      ...
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(25000, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      await borrowerOperations.openTrove(
        ...
        dec(400, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(40000, 18)),
        defaulter_4,
        defaulter_4,
        { from: defaulter_4 }
      );
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "17832817337461300000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "5944272445820430000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "4458204334365320000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "11764705882352900000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(alice)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(bob)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(carol)).toString(), "0"), 1000);
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      */
      ...
      const dennis_ETHWithdrawn = th
        ...
        .toString();
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "27692307692307700000000"),
        100000000000
      );
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "1672240802675590000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "2090301003344480000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "1045150501672240000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 KUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      const dennis_ETHWithdrawn = th
        ...
        .toString();
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "1666666666666666666666"),
        100000
      );
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(alice)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "2000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Defaulter 1 liquidated. 20000 KUSD fully offset with pool.
      await troveManager.liquidate(assetAddress1, defaulter_1, { from: owner });

      // Carol, Dennis each deposit 10000 KUSD
      ...
      // await stabilityPool.provideToSP(dec(1, 18), assetAddress1, { from: erin })
      ...
      // Expect Alice And Bob's compounded deposit to be 0 KUSD
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(bob)).toString(), "0"), 10000);
      ...
      // Expect Carol And Dennis' compounded deposit to be 50 KUSD
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
    ...
    it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      ...
      // Defaulter 1 liquidated. 10--0 KUSD fully offset, Pool remains non-zero
      await troveManager.liquidate(assetAddress1, defaulter_1, { from: owner });
      ...
      // Defaulter 2 liquidated. 1--00 KUSD, empties pool
      await troveManager.liquidate(assetAddress1, defaulter_2, { from: owner });
      ...
      // Defaulter 4 liquidated. 10000 KUSD, empties pool
      await troveManager.liquidate(assetAddress1, defaulter_4, { from: owner });
    ...
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const erin_ETHWithdrawn = th
        .getEventArgByName(txE, EVENT_ASSET_GAIN_NAME, EVENT_ASSET_GAIN_PARAM)
        .toString();
      ...
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), "8333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "16666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(erin)).toString(), "25000000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1, L2, L3 liquidated with 10000 KUSD each
    // A withdraws all
    ...
    // A,B deposit 10000 KUSD
    // L1 cancels 20000 KUSD, 2E
    // B,C deposits 10000 KUSD
    // L2 cancels 20000 KUSD, 2E
    // E,F deposit 10000 KUSD
    // L3 cancels 20000, 200E
    // G,H deposits 10000
    // L4 cancels 20000, 200E

    // Expect all depositors withdraw 0 KUSD and 100 ETH
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Alice, Bob each deposit 10k KUSD
      ...
      // Erin, Flyn each deposit 10000 KUSD
      ...
      // Graham, Harriet each deposit 10000 KUSD
      ...
      const flyn_ETHWithdrawn = th
        .getEventArgByName(txF, EVENT_ASSET_GAIN_NAME, EVENT_ASSET_GAIN_PARAM)
        .toString();
      const graham_ETHWithdrawn = th
        .getEventArgByName(txG, EVENT_ASSET_GAIN_NAME, EVENT_ASSET_GAIN_PARAM)
        .toString();
      const harriet_ETHWithdrawn = th
        .getEventArgByName(txH, EVENT_ASSET_GAIN_NAME, EVENT_ASSET_GAIN_PARAM)
        .toString();

      // Expect all deposits to be 0 KUSD
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(dennis)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(erin)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(flyn)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await kusdToken.balanceOf(graham)).toString(), "0"), 100000);
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(harriet)).toString(), "0"),
        100000
      );
    ...
    // A withdraws all
    // B deposits 10000
    // L2 of 9900 KUSD, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "9999999910000000000000"),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      ...
      await borrowerOperations.openTrove(
        ...
        dec(60, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(9900, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      // Expect Bob to withdraw 1% of initial deposit (100 KUSD) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), "100000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 KUSD
    // A withdraws all
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(330, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "59400000000000000000000"),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      Bob:  1000 KUSD, 55 Ether
      Carol:  2000 KUSD, 110 Ether
      Dennis:  3000 KUSD, 165 Ether
     
      Total: 6000 KUSD, 300 Ether
      */
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), dec(100, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), dec(200, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), dec(300, 18)),
        100000
      );
    ...
    // A make deposit 10000 KUSD
    // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 KUSD
    // A withdraws
    // B makes deposit 10000 KUSD
    // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 KUSD total deposits)
    // B withdraws
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(99999, 17)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 KUSD and the full ETH gain of 100 ether
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
    ...
    // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 KUSD total deposits)
    // B withdraws
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire ETH gain from L2
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(600, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "59999400000000000000000"),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      ...
      // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} KUSD
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(carol)).toString(), dec(2, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), dec(3, 17)),
        100000
      );
    ...
    // A make deposit 10000 KUSD
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 KUSD
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, "9999999999900000000000"),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      ...
      const aliceDeposit = (await stabilityPool.getCompoundedKUSDDeposit(alice)).toString();
    ...
    /* A make deposit 10000 KUSD
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 KUSD, 1 ETH
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 KUSD, 1 ETH
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 KUSD, 1 ETH
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 KUSD, 1 ETH
    expect A, B, C, D each withdraw ~100 Ether
    */
    it("withdrawFromSP(): Several deposits of 10000 KUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100000, "ether"),
      ...
      assert.equal(await kusdToken.balanceOf(alice), "0");
      assert.equal(await kusdToken.balanceOf(alice), "0");
      assert.equal(await kusdToken.balanceOf(alice), "0");
      // D should withdraw around 0.9999 KUSD, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await kusdToken.balanceOf(dennis)).toString(), dec(99999, 12)),
        100000
      );
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(200, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(24100, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      await borrowerOperations.openTrove(
        ...
        dec(200, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(24300, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      );
      await borrowerOperations.openTrove(
        ...
        dec(200, "ether"),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(24500, 18)),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3 }
      );
      ...
      const A_deposit = await stabilityPool.getCompoundedKUSDDeposit(A);
      const B_deposit = await stabilityPool.getCompoundedKUSDDeposit(B);
      ...
      const KUSDinSP_1 = await stabilityPool.getTotalKUSDDeposits();
      // console.log(`KUSDinSP_1: ${KUSDinSP_1}`)
      assert.equal(KUSDinSP_1, "0");

      // Check SP KUSD balance is zero
      const SPKUSDBalance_1 = await kusdToken.balanceOf(stabilityPool.address);
      // console.log(`SPKUSDBalance_1: ${SPKUSDBalance_1}`)
      assert.equal(SPKUSDBalance_1, "0");
      ...
      // Check compounded deposits
      const C_deposit = await stabilityPool.getCompoundedKUSDDeposit(C);
      const D_deposit = await stabilityPool.getCompoundedKUSDDeposit(D);
      ...
      // Check SP tracker is zero
      const KUSDinSP_2 = await stabilityPool.getTotalKUSDDeposits();
      // console.log(`KUSDinSP_2: ${KUSDinSP_2}`)
      assert.equal(KUSDinSP_2, "0");

      // Check SP KUSD balance is zero
      const SPKUSDBalance_2 = await kusdToken.balanceOf(stabilityPool.address);
      // console.log(`SPKUSDBalance_2: ${SPKUSDBalance_2}`)
      assert.equal(SPKUSDBalance_2, "0");
      ...
      // Check compounded deposits
      const E_deposit = await stabilityPool.getCompoundedKUSDDeposit(E);
      const F_deposit = await stabilityPool.getCompoundedKUSDDeposit(F);
      ...
      // Check SP tracker is zero
      const KUSDinSP_3 = await stabilityPool.getTotalKUSDDeposits();
      assert.equal(KUSDinSP_3, "0");

      // Check SP KUSD balance is zero
      const SPKUSDBalance_3 = await kusdToken.balanceOf(stabilityPool.address);
      // console.log(`SPKUSDBalance_3: ${SPKUSDBalance_3}`)
      assert.equal(SPKUSDBalance_3, "0");
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 9999.9999999 KUSD
      await borrowerOperations.openTrove(
        assetAddress1,
        dec(100, "ether"),
        th._100pct,
      ...
      const alice_ETHGainAt2ndScaleChange = (
        await stabilityPool.getDepositorAssetGain(alice)
      ).toString();

      // E deposits 9999.9 KUSD
      await kusdToken.transfer(erin, dec(99999, 17), { from: whale });
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      await priceFeed.setPrice(assetAddress1, dec(2, 27));
      ...
      await borrowerOperations.openTrove(
        ...
        dec(1, 27),
        th._100pct,
        await getOpenTroveKUSDAmount(assetAddress1, dec(1, 36)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      );
      ...
      const alice_ETHWithdrawn = th.getEventArgByName(
        txA,
        EVENT_ASSET_GAIN_NAME,
        EVENT_ASSET_GAIN_PARAM
      );
      ...
      // Check KUSD balances
      const aliceKUSDBalance = await kusdToken.balanceOf(alice);
      const aliceExpectedKUSDBalance = web3.utils.toBN(dec(5, 35));
      const aliceKUSDBalDiff = aliceKUSDBalance.sub(aliceExpectedKUSDBalance).abs();

      assert.isTrue(aliceKUSDBalDiff.lte(toBN(dec(1, 18)))); // error tolerance of 1e18

      const bobKUSDBalance = await kusdToken.balanceOf(bob);
      const bobExpectedKUSDBalance = toBN(dec(5, 35));
      const bobKUSDBalDiff = bobKUSDBalance.sub(bobExpectedKUSDBalance).abs();

      assert.isTrue(bobKUSDBalDiff.lte(toBN(dec(1, 18))));
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      const aliceKUSDBalance = await kusdToken.balanceOf(alice);
      const aliceExpectedKUSDBalance = toBN("99999999999999997500000000000000000000");
      const aliceKUSDBalDiff = aliceKUSDBalance.sub(aliceExpectedKUSDBalance).abs();

      assert.isTrue(aliceKUSDBalDiff.lte(toBN(dec(1, 18))));

      const bobKUSDBalance = await kusdToken.balanceOf(bob);
      const bobExpectedKUSDBalance = toBN("99999999999999997500000000000000000000");
      const bobKUSDBalDiff = bobKUSDBalance.sub(bobExpectedKUSDBalance).abs();

      assert.isTrue(bobKUSDBalDiff.lte(toBN("100000000000000000000")));
