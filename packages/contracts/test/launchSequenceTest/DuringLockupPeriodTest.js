...
contract("During the initial lockup period", async accounts => {
  ...
  describe("KUMO transfer during first year after KUMO deployment", async accounts => {
    ...
    it("Kumo multisig can not transfer KUMO to a LC that was deployed directly", async () => {
      ...
      const LC_A = await LockupContract.new(kumoToken.address, A, oneYearFromSystemDeployment, {
        from: multisig
      });

      // Account F deploys LC_B
      const LC_B = await LockupContract.new(kumoToken.address, B, oneYearFromSystemDeployment, {
        from: F
      });

      // KUMO deployer deploys LC_C
      const LC_C = await LockupContract.new(kumoToken.address, A, oneYearFromSystemDeployment, {
        from: liquityAG
      });

      // Kumo multisig attempts KUMO transfer to LC_A
      try {
        const KUMOtransferTx_A = await kumoToken.transfer(LC_A.address, dec(1, 18), {
          from: multisig
        });
      ...
      try {
        const KUMOtransferTx_B = await kumoToken.transfer(LC_B.address, dec(1, 18), {
          from: multisig
        });
      ...
      try {
        const KUMOtransferTx_C = await kumoToken.transfer(LC_C.address, dec(1, 18), {
          from: multisig
        });
    ...
    it("Kumo multisig can not transfer to an EOA or Kumo system contracts", async () => {
      ...
      for (const contract of Object.keys(coreContracts)) {
        const KUMOtransferTxPromise = kumoToken.transfer(
          coreContracts[contract].address,
          dec(1, 18),
          { from: multisig }
        );
        await assertRevert(
          KUMOtransferTxPromise,
          "KUMOToken: recipient must be a LockupContract registered in the Factory"
        );
      }
      ...
      for (const contract of Object.keys(KUMOContracts)) {
        const KUMOtransferTxPromise = kumoToken.transfer(
          KUMOContracts[contract].address,
          dec(1, 18),
          { from: multisig }
        );
        await assertRevert(
    ...
    it("Kumo multisig can not approve any EOA or Kumo system contract to spend their KUMO", async () => {
      ...
      for (const contract of Object.keys(coreContracts)) {
        const KUMOApproveTxPromise = kumoToken.approve(coreContracts[contract].address, dec(1, 18), {
          from: multisig
        });
      ...
      for (const contract of Object.keys(KUMOContracts)) {
        const KUMOApproveTxPromise = kumoToken.approve(KUMOContracts[contract].address, dec(1, 18), {
          from: multisig
        });
        await assertRevert(KUMOApproveTxPromise, "KUMOToken: caller must not be the multisig");
      }
    ...
    it("Kumo multisig can not increaseAllowance for any EOA or Kumo contract", async () => {
      // Multisig attempts to approve EOAs to spend KUMO
      const KUMOIncreaseAllowanceTxPromise_1 = kumoToken.increaseAllowance(A, dec(1, 18), {
        from: multisig
      });
      const KUMOIncreaseAllowanceTxPromise_2 = kumoToken.increaseAllowance(B, dec(1, 18), {
        from: multisig
      });
      await assertRevert(
        KUMOIncreaseAllowanceTxPromise_1,
        "KUMOToken: caller must not be the multisig"
      );
      await assertRevert(
        KUMOIncreaseAllowanceTxPromise_2,
        "KUMOToken: caller must not be the multisig"
      );

      // Multisig attempts to approve Kumo contracts to spend KUMO
      for (const contract of Object.keys(coreContracts)) {
        const KUMOIncreaseAllowanceTxPromise = kumoToken.increaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: multisig }
        );
        await assertRevert(
          KUMOIncreaseAllowanceTxPromise,
          "KUMOToken: caller must not be the multisig"
        );
      }
    ...
    it("Kumo multisig can not decreaseAllowance for any EOA or Kumo contract", async () => {
      ...
      const KUMODecreaseAllowanceTxPromise_1 = kumoToken.decreaseAllowance(A, dec(1, 18), {
        from: multisig
      });
      const KUMODecreaseAllowanceTxPromise_2 = kumoToken.decreaseAllowance(B, dec(1, 18), {
        from: multisig
      });
      await assertRevert(
        KUMODecreaseAllowanceTxPromise_1,
        "KUMOToken: caller must not be the multisig"
      );
      await assertRevert(
        KUMODecreaseAllowanceTxPromise_2,
        "KUMOToken: caller must not be the multisig"
      );

      // Multisig attempts to decrease allowance on Kumo contracts
      for (const contract of Object.keys(coreContracts)) {
        const KUMODecreaseAllowanceTxPromise = kumoToken.decreaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: multisig }
        );
        await assertRevert(
          KUMODecreaseAllowanceTxPromise,
          "KUMOToken: caller must not be the multisig"
        );
      }
    ...
    it("Kumo multisig can not be the sender in a transferFrom() call", async () => {
      ...
      const KUMOtransferFromTxPromise_1 = kumoToken.transferFrom(multisig, A, dec(1, 18), {
        from: A
      });
      const KUMOtransferFromTxPromise_2 = kumoToken.transferFrom(multisig, C, dec(1, 18), {
        from: B
      });
    ...
    it("Anyone (other than Kumo multisig) can transfer KUMO to LCs deployed by anyone directly", async () => {
      ...
      const LC_C = await LockupContract.new(kumoToken.address, C, oneYearFromSystemDeployment, {
        from: multisig
      });
    ...
    it("Anyone (other than liquity multisig) can increaseAllowance for any EOA or Kumo contract", async () => {
      ...
      const KUMOIncreaseAllowanceTx_1 = await kumoToken.increaseAllowance(A, dec(1, 18), {
        from: F
      });
      const KUMOIncreaseAllowanceTx_2 = await kumoToken.increaseAllowance(B, dec(1, 18), {
        from: G
      });
      ...
      for (const contract of Object.keys(coreContracts)) {
        const KUMOIncreaseAllowanceTx = await kumoToken.increaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
    ...
    it("Anyone (other than liquity multisig) can decreaseAllowance for any EOA or Kumo contract", async () => {
      ...
      for (const contract of Object.keys(coreContracts)) {
        const KUMOtransferTx = await kumoToken.increaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
      ...
      const KUMODecreaseAllowanceTx_1 = await kumoToken.decreaseAllowance(A, dec(1, 18), {
        from: F
      });
      const KUMODecreaseAllowanceTx_2 = await kumoToken.decreaseAllowance(B, dec(1, 18), {
        from: G
      });
      ...
      for (const contract of Object.keys(coreContracts)) {
        const KUMODecreaseAllowanceTx = await kumoToken.decreaseAllowance(
          coreContracts[contract].address,
          dec(1, 18),
          { from: F }
        );
  ...
  describe("Transferring KUMO to LCs", async accounts => {
    it("Kumo multisig can transfer KUMO (vesting) to lockup contracts they deployed", async () => {
      ...
      assert.isTrue(
        KUMOBalanceOfLC_T1_1.eq(th.toBN(initialKUMOBalanceOfLC_T1).add(th.toBN(dec(1, 24))))
      );
      assert.isTrue(
        KUMOBalanceOfLC_T2_1.eq(th.toBN(initialKUMOBalanceOfLC_T2).add(th.toBN(dec(1, 24))))
      );
      assert.isTrue(
        KUMOBalanceOfLC_T3_1.eq(th.toBN(initialKUMOBalanceOfLC_T3).add(th.toBN(dec(1, 24))))
      );
  ...
  describe("Deploying new LCs", async accounts => {
    ...
    it("KUMO Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(kumoToken.address, B, twoYearsFromSystemDeployment, {
        from: liquityAG
      });
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash);

      const LC_C = await LockupContract.new(kumoToken.address, C, twoYearsFromSystemDeployment, {
        from: liquityAG
      });
    ...
    it("Anyone can deploy LCs with unlockTime = one year from deployment, directly and through factory", async () => {
      // Deploy directly
      const LC_1 = await LockupContract.new(kumoToken.address, A, oneYearFromSystemDeployment, {
        from: D
      });
      const LCTxReceipt_1 = await web3.eth.getTransactionReceipt(LC_1.transactionHash);

      const LC_2 = await LockupContract.new(kumoToken.address, B, oneYearFromSystemDeployment, {
        from: liquityAG
      });
      const LCTxReceipt_2 = await web3.eth.getTransactionReceipt(LC_2.transactionHash);

      const LC_3 = await LockupContract.new(kumoToken.address, C, oneYearFromSystemDeployment, {
        from: multisig
      });
    ...
    it("Anyone can deploy LCs with unlockTime > one year from deployment, directly and through factory", async () => {
      ...
      // Deploy directly
      const LC_1 = await LockupContract.new(kumoToken.address, A, twoYearsFromSystemDeployment, {
        from: D
      });
      const LCTxReceipt_1 = await web3.eth.getTransactionReceipt(LC_1.transactionHash);

      const LC_2 = await LockupContract.new(kumoToken.address, B, justOverOneYear, {
        from: multisig
      });
      const LCTxReceipt_2 = await web3.eth.getTransactionReceipt(LC_2.transactionHash);

      const LC_3 = await LockupContract.new(kumoToken.address, E, _17YearsFromDeployment, {
        from: E
      });
    ...
    it("No one can deploy LCs with unlockTime < one year from deployment, directly or through factory", async () => {
      ...
      const directDeploymentTxPromise_2 = LockupContract.new(kumoToken.address, B, "43200", {
        from: multisig
      });
      const directDeploymentTxPromise_3 = LockupContract.new(kumoToken.address, E, "354534", {
        from: E
      });
