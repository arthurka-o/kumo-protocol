...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      const txB = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: bob
      });
      const txC = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: carol
      });
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(alice)).toString(),
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(bob)).toString(),
          "6666666666666666666666"
        ),
        10000
      );
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(carol)).toString(),
          "6666666666666666666666"
        ),
        10000
      );
    ...
    it("withdrawAssetGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(alice)).toString(), "0"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(bob)).toString(), "0"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(carol)).toString(), "0"),
        10000
      );
    ...
    it("withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      ...
      const txD = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: dennis
      });
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(dennis)).toString(),
          "5000000000000000000000"
        ),
        100000
      );
    ...
    it("withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 KUSD.  A, B, C, D withdraw correct KUSD deposit and ETH Gain", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(dennis)).toString(), "0"),
        100000
      );
    ...
    it("withdrawAssetGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const txE = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: erin
      });
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(erin)).toString(),
    ...
    it("withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const txF = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: flyn
      });
      const txG = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: graham
      });
      const txH = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: harriet
      });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(erin)).toString(), "0"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(flyn)).toString(), "0"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(graham)).toString(), "0"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(harriet)).toString(), "0"),
        100000
      );
    ...
    it("withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Expect Bob to retain 1% of initial deposit (100 KUSD) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(bob)).toString(),
    ...
    it("withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedKUSDDeposit(bob)).toString(), dec(1, 17)),
        100000
      );
    ...
    */
    it("withdrawAssetGainToTrove(): Several deposits of 10000 KUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      assert.equal(await stabilityPool.getCompoundedKUSDDeposit(alice), "0");
      assert.equal(await stabilityPool.getCompoundedKUSDDeposit(alice), "0");
      assert.equal(await stabilityPool.getCompoundedKUSDDeposit(alice), "0");
      // D should retain around 0.9999 KUSD, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedKUSDDeposit(dennis)).toString(),
          dec(99999, 12)
    ...
    it("withdrawAssetGainToTrove(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      // Check KUSD balances
      const aliceKUSDBalance = await stabilityPool.getCompoundedKUSDDeposit(alice);
      ...
      const bobKUSDBalance = await stabilityPool.getCompoundedKUSDDeposit(bob);
    ...
    it("withdrawAssetGainToTrove(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      const txAPromise = stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: alice
      });
      const txBPromise = stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: bob
      });
