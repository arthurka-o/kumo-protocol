...
contract("KUMO Token", async accounts => {
  ...
  it("approve(): reverts when owner param is address(0)", async () => {
    ...
    const txPromise = kumoTokenTester.callInternalApprove(ZERO_ADDRESS, A, dec(100, 18), {
      from: B
    });
    await assertRevert(txPromise);
  });
  ...
  it("transfer(): transfer to or from the zero-address reverts", async () => {
    ...
    const txPromiseFromZero = kumoTokenTester.callInternalTransfer(ZERO_ADDRESS, A, dec(100, 18), {
      from: B
    });
    const txPromiseToZero = kumoTokenTester.callInternalTransfer(A, ZERO_ADDRESS, dec(100, 18), {
      from: B
    });
  ...
  it("Initializes DOMAIN_SEPARATOR correctly", async () => {
    assert.equal(
      await kumoTokenTester.domainSeparator(),
  ...
  it("permit(): permits and emits an Approval event (replay protected)", async () => {
    ...
    await assertRevert(
      kumoTokenTester.permit(approve.owner, approve.spender, approve.value, deadline, v, r, s),
      "KUMO: invalid signature"
    );

    // Check that the zero address fails
    await assertRevert(
      kumoTokenTester.permit(
