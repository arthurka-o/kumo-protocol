...
contract("BorrowerOperations", async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): reverts when top-up would leave trove with ICR < MCR", async () => {
      ...
      assert.isTrue(
        (await troveManager.getCurrentICR(assetAddress1, alice, price)).lt(toBN(dec(110, 16)))
      );

      const collTopUp = 1; // 1 wei top up

      await assertRevert(
        borrowerOperations.addColl(assetAddress1, dec(1, "ether"), alice, alice, {
          from: alice,
          value: collTopUp
        }),
    ...
    it("addColl(), active Trove: adds the correct collateral amount to the Trove", async () => {
      ...
      const alice_Trove_Before = await troveManager.Troves(assetAddress1, alice);
    ...
    it("addColl(), active Trove: updates the stake and updates the total stakes", async () => {
      ...
      await borrowerOperations.addColl(assetAddress1, dec(2, "ether"), alice, alice, {
        from: alice
      });
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_KUSDDebt snapshots", async () => {
      ...
      const alicePendingKUSDDebtReward = await troveManager.getPendingKUSDDebtReward(
        assetAddress1,
        alice
      );
      const bobPendingKUSDDebtReward = await troveManager.getPendingKUSDDebtReward(
        assetAddress1,
        bob
      );
      for (reward of [
        ...
        bobPendingETHReward,
        alicePendingKUSDDebtReward,
        bobPendingKUSDDebtReward
      ]) {
      ...
      const alice_rewardSnapshot_After = await troveManager.rewardSnapshots(assetAddress1, alice);
      ...
      const bob_rewardSnapshot_After = await troveManager.rewardSnapshots(assetAddress1, bob);
    ...
    it("addColl(), reverts if trove is non-existent or closed", async () => {
      ...
      try {
        const txBob = await borrowerOperations.addColl(assetAddress1, dec(1, "ether"), bob, bob, {
          from: bob,
          value: dec(1, "ether")
        });
    ...
    it("addColl(): can add collateral in Recovery Mode", async () => {
      ...
      const aliceCollAfter = toBN((await troveManager.Troves(assetAddress1, alice))[2]);
    ...
    it("withdrawColl(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        borrowerOperations.withdrawColl(assetAddress1, 1, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    });
    ...
    it("withdrawColl(): reverts when system is in Recovery Mode", async () => {
      ...
      const txAlice = await borrowerOperations.withdrawColl(assetAddress1, 1000, alice, alice, {
        from: alice
      });
      ...
      try {
        const txBob = await borrowerOperations.withdrawColl(assetAddress1, 1000, bob, bob, {
          from: bob
        });
        assert.isFalse(txBob.receipt.status);
      } catch (err) {
    ...
    it("withdrawColl(): reverts when withdrawal would bring the user's ICR < MCR", async () => {
      ...
      try {
        const txBob = await borrowerOperations.withdrawColl(assetAddress1, 1, bob, bob, {
          from: bob
        });
        assert.isFalse(txBob.receipt.status);
      } catch (err) {
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      try {
        const txData = await borrowerOperations.withdrawColl(assetAddress1, "1", alice, alice, {
          from: alice
        });
    ...
    it("withdrawColl(): leaves the Trove active when the user withdraws less than all the collateral", async () => {
      ...
      await borrowerOperations.withdrawColl(assetAddress1, dec(100, "finney"), alice, alice, {
        from: alice
      });
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_KUSDDebt snapshots", async () => {
      ...
      await borrowerOperations.withdrawColl(assetAddress1, aliceCollWithdrawal, alice, alice, {
        from: alice
      });
      await borrowerOperations.withdrawColl(assetAddress1, bobCollWithdrawal, bob, bob, {
        from: bob
      });
    ...
    it("withdrawKUSD(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, KUSDwithdrawal, alice, alice, {
          from: alice
        }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    ...
    it("withdrawKUSD(): decays a non-zero base rate", async () => {
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(20, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      // D withdraws KUSD
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(1, 18), A, A, {
        from: D
      });
    ...
    it("withdrawKUSD(): reverts if max fee > 100%", async () => {
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(40, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, dec(2, 18), dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, "1000000000000000001", dec(1, 18), A, A, {
          from: A
        }),
        "Max fee percentage must be between 0.5% and 100%"
      );
    ...
    it("withdrawKUSD(): reverts if max fee < 0.5% in Normal mode", async () => {
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, 0, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, 1, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, "4999999999999999", dec(1, 18), A, A, {
          from: A
        }),
        "Max fee percentage must be between 0.5% and 100%"
      );
    ...
    it("withdrawKUSD(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(70, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(80, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D }
      });
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(180, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E }
      });
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, lessThan5pct, dec(3, 18), A, A, { from: A }),
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, dec(1, 16), dec(1, 18), A, A, { from: B }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, dec(3754, 13), dec(1, 18), A, A, { from: C }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawKUSD(assetAddress1, dec(5, 15), dec(1, 18), A, A, { from: D }),
        "Fee exceeded provided maximum"
      );
    });
    ...
    it("withdrawKUSD(): succeeds when fee is less than max fee percentage", async () => {
      ...
      const tx1 = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        moreThan5pct,
        dec(1, 18),
        A,
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        dec(5, 16),
        dec(1, 18),
        A,
      ...
      const tx3 = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        dec(1, 17),
        dec(1, 18),
        A,
      ...
      const tx4 = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        dec(37659, 13),
        dec(1, 18),
        A,
      ...
      const tx5 = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        dec(1, 18),
        dec(1, 18),
        A,
    ...
    it("withdrawKUSD(): doesn't change base rate if it is already zero", async () => {
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(50, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
      ...
      // D withdraws KUSD
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(37, 18), A, A, {
        from: D
      });
      ...
      // E opens trove
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(12, 18), A, A, {
        from: E
      });
    ...
    it("withdrawKUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(1, 18), C, C, {
        from: C
      });
    ...
    it("withdrawKUSD(): borrowing at non-zero base rate sends KUSD fee to KUMO staking contract", async () => {
      ...
      // D withdraws KUSD
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(37, 18), C, C, {
        from: D
      });
    ...
    if (!withProxy) {
      ...
      it("withdrawKUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const withdrawalTx = await borrowerOperations.withdrawKUSD(
          assetAddress1,
          th._100pct,
          toBN(dec(37, 18)),
          D,
        ...
        const newDebt = (await troveManager.Troves(assetAddress1, D))[TroveData.debt];
    ...
    it("withdrawKUSD(): Borrowing at non-zero base rate increases the KUMO staking contract KUSD fees-per-unit-staked", async () => {
      ...
      // D withdraws KUSD
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, toBN(dec(37, 18)), D, D, {
        from: D
      });
    ...
    it("withdrawKUSD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, D_KUSDRequest, D, D, {
        from: D
      });
    ...
    it("withdrawKUSD(): Borrowing at zero base rate changes KUSD fees-per-unit-staked", async () => {
      ...
      // D withdraws KUSD
      await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, dec(37, 18), D, D, {
        from: D
      });
    ...
    it("withdrawKUSD(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.withdrawKUSD(
        assetAddress1,
        th._100pct,
        dec(100, 18),
        bob,
      ...
      try {
        const txCarol = await borrowerOperations.withdrawKUSD(
          assetAddress1,
          th._100pct,
          dec(100, 18),
          carol,
    ...
    it("withdrawKUSD(): reverts when requested withdrawal amount is zero KUSD", async () => {
      ...
      const txBob = await borrowerOperations.withdrawKUSD(assetAddress1, th._100pct, 1, bob, bob, {
        from: bob
      });
      assert.isTrue(txBob.receipt.status);
      ...
      try {
        const txAlice = await borrowerOperations.withdrawKUSD(
          assetAddress1,
          th._100pct,
          0,
          alice,
    ...
    it("withdrawKUSD(): reverts if system is in Recovery Mode", async () => {
      ...
      try {
        const txData = await borrowerOperations.withdrawKUSD(
          assetAddress1,
          th._100pct,
          "200",
          alice,
    ...
    it("withdrawKUSD(): increases the Trove's KUSD debt by the correct amount", async () => {
      ...
      await borrowerOperations.withdrawKUSD(
        assetAddress1,
        th._100pct,
        await getNetBorrowingAmount(100, assetAddress1),
        alice,
        alice,
        { from: alice }
      );
    ...
    it("withdrawKUSD(): increases KUSD debt in ActivePool by correct amount", async () => {
      ...
      await borrowerOperations.withdrawKUSD(
        assetAddress1,
        th._100pct,
        await getNetBorrowingAmount(dec(10000, 18), assetAddress1),
        alice,
        alice,
        { from: alice }
      );
    ...
    it("withdrawKUSD(): increases user KUSDToken balance by correct amount", async () => {
      await openTrove({
        asset: assetAddress1,
        tokenAmount: toBN(dec(100, "ether")),
        extraParams: { from: alice }
      });
      ...
      assert.isTrue(
        alice_KUSDTokenBalance_After.eq(alice_KUSDTokenBalance_Before.add(toBN(dec(10000, 18))))
      );
    });
    ...
    it("repayKUSD(): reverts when repayment would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        borrowerOperations.repayKUSD(assetAddress1, KUSDRepayment, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    });
    ...
    it("repayKUSD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN("2")), assetAddress1),
        A,
        A,
        { from: A }
      );
      ...
      const repayTxB = await borrowerOperations.repayKUSD(assetAddress1, dec(19, 25), B, B, {
        from: B
      });
    ...
    it("repayKUSD(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.repayKUSD(assetAddress1, dec(10, 18), bob, bob, {
        from: bob
      });
      assert.isTrue(txBob.receipt.status);
      ...
      try {
        const txCarol = await borrowerOperations.repayKUSD(
          assetAddress1,
          dec(10, 18),
          carol,
          carol,
    ...
    it("repayKUSD(): reverts when attempted repayment is > the debt of the trove", async () => {
      ...
      try {
        const txAlice = await borrowerOperations.repayKUSD(
          assetAddress1,
    ...
    it("repayKUSD(): reduces the Trove's KUSD debt by the correct amount", async () => {
      ...
      await borrowerOperations.repayKUSD(
        assetAddress1,
    ...
    it("repayKUSD(): decreases KUSD debt in ActivePool by correct amount", async () => {
      ...
      th.assertIsApproximatelyEqual(
        activePool_KUSD_After,
        activePool_KUSD_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });
    ...
    it("repayKUSD(): decreases user KUSDToken balance by correct amount", async () => {
      ...
      const alice_KUSDTokenBalance_After = await kusdToken.balanceOf(alice);
      th.assertIsApproximatelyEqual(
        alice_KUSDTokenBalance_After,
        alice_KUSDTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });
    ...
    it("repayKUSD(): can repay debt in Recovery Mode", async () => {
      ...
      const tx = await borrowerOperations.repayKUSD(
        assetAddress1,
        aliceDebtBefore.div(toBN(10)),
        alice,
        alice,
    ...
    it("repayKUSD(): Reverts if borrower has insufficient KUSD balance to cover his debt repayment", async () => {
      ...
      const repayKUSDPromise_B = borrowerOperations.repayKUSD(
        assetAddress1,
        toBN(dec(6, 18)),
        B,
        B,
    ...
    it("adjustTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(assetAddress1, 0, 0, 0, dec(1, 18), true, A, A, {
          from: A,
          value: dec(2, 16)
        }),
        "Max fee percentage must be between 0.5% and 100%"
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(assetAddress1, 0, 1, 0, dec(1, 18), true, A, A, {
          from: A,
          value: dec(2, 16)
        }),
        "Max fee percentage must be between 0.5% and 100%"
    ...
    it("adjustTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      // Borrower C triggers a fee
      await borrowerOperations.adjustTrove(assetAddress1, 0, th._100pct, 0, dec(1, 18), true, C, C, {
        from: C
      });
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends KUSD fee to KUMO staking contract", async () => {
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(37, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D }
      });
    ...
    if (!withProxy) {
      ...
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const D_newDebt = toBN((await troveManager.Troves(assetAddress1, D))[TroveData.debt]);
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(200000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: bob }
      });
      ...
      const emittedFee = toBN(
        await th.getEventArgByName(txAlice, "KUSDBorrowingFeePaid", "_KUSDFee")
      );
    ...
    it("adjustTrove(): reverts when KUSD repaid is > debt of the trove", async () => {
      ...
      const remainingDebt = (await troveManager.getTroveDebt(assetAddress1, bob)).sub(
        KUSD_GAS_COMPENSATION
      );
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      await borrowerOperations.adjustTrove(
        ...
        th._100pct,
        dec(1, 17),
        await getNetBorrowingAmount(dec(1, 18), assetAddress1),
        true,
        alice,
        alice,
        { from: alice }
    ...
    it("adjustTrove(): changes KUSDToken balance by the requested decrease", async () => {
      ...
      const alice_KUSDTokenBalance_After = await kusdToken.balanceOf(alice);
      assert.isTrue(
        alice_KUSDTokenBalance_After.eq(alice_KUSDTokenBalance_Before.sub(toBN(dec(10, 18))))
      );
    });
    ...
    it("adjustTrove(): changes KUSDToken balance by the requested increase", async () => {
      ...
      const alice_KUSDTokenBalance_After = await kusdToken.balanceOf(alice);
      assert.isTrue(
        alice_KUSDTokenBalance_After.eq(alice_KUSDTokenBalance_Before.add(toBN(dec(100, 18))))
      );
    });
    ...
    it("adjustTrove(): Changes the KUSD debt in ActivePool by requested increase", async () => {
      ...
      await borrowerOperations.adjustTrove(
        ...
        th._100pct,
        0,
        await getNetBorrowingAmount(dec(100, 18), assetAddress1),
        true,
        alice,
        alice,
        { from: alice, value: dec(1, "ether") }
      ...
      th.assertIsApproximatelyEqual(
        activePool_KUSDDebt_After,
        activePool_KUSDDebt_Before.add(toBN(dec(100, 18)))
      );
    });
    ...
    it("adjustTrove(): Reverts if requested debt increase and amount is zero", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(assetAddress1, 0, th._100pct, 0, 0, true, alice, alice, {
          from: alice
        }),
    ...
    it("adjustTrove(): Reverts if it’s zero adjustment", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(assetAddress1, 0, th._100pct, 0, 0, false, alice, alice, {
          from: alice
        }),
    ...
    it("adjustTrove(): Reverts if borrower has insufficient KUSD balance to cover his debt repayment", async () => {
      ...
      const repayKUSDPromise_B = borrowerOperations.adjustTrove(
        assetAddress1,
        0,
        th._100pct,
        0,
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      ...
      await assertRevert(
        borrowerOperations.closeTrove(assetAddress1, { from: carol }),
    ...
    it("closeTrove(): sets Trove's stake to zero", async () => {
      ...
      const stakeAfter = (await troveManager.Troves(assetAddress1, alice))[2].toString();
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_ETH_A_Snapshot = toBN((await troveManager.rewardSnapshots(assetAddress1, alice))[0]);
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's KUSDToken balance", async () => {
      ...
      th.assertIsApproximatelyEqual(
        alice_KUSDBalance_After,
        alice_KUSDBalance_Before.sub(aliceDebt.sub(KUSD_GAS_COMPENSATION))
      );
    });
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      const [liquidatedDebt_C, liquidatedColl_C] = await th.getEmittedLiquidationValues(
        liquidationTx
      );
      ...
      const defaultPool_KUSDDebt = await defaultPool.getKUSDDebt(assetAddress1);
      ...
      const defaultPool_KUSDDebt_afterAliceCloses = await defaultPool.getKUSDDebt(assetAddress1);
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPool_KUSDDebt_afterAliceCloses,
          defaultPool_KUSDDebt.sub(pendingDebtReward_A)
        ),
        1000
      );
      ...
      const defaultPool_KUSDDebt_afterBobCloses = await defaultPool.getKUSDDebt(assetAddress1);
    ...
    it("openTrove(): Opens a trove with net debt >= minimum net debt", async () => {
      ...
      const txA = await borrowerOperations.openTrove(
        ...
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1)), assetAddress1),
        A,
        A,
        { from: A }
      );
      ...
      const txC = await borrowerOperations.openTrove(
        ...
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(dec(47789898, 22))), assetAddress1),
        A,
        A,
        { from: C }
      );
    ...
    it("openTrove(): reverts if net debt < minimum net debt", async () => {
      const txAPromise = borrowerOperations.openTrove(assetAddress1, 0, th._100pct, 0, A, A, {
        from: A,
        value: dec(100, 30)
      });
      ...
      const txBPromise = borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.sub(toBN(1)), assetAddress1),
        B,
        B,
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      await openTrove({
        asset: assetAddress1,
        extraKUSDAmount: toBN(dec(12, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E }
      });
    ...
    it("openTrove(): reverts if max fee > 100%", async () => {
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, dec(2, 18), dec(10000, 18), A, A, {
          from: A,
      ...
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, "1000000000000000001", dec(20000, 18), B, B, {
          from: B,
          value: dec(1000, "ether")
        }),
        "Max fee percentage must be between 0.5% and 100%"
    ...
    it("openTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, 0, dec(195000, 18), A, A, {
          from: A,
      ...
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, 1, dec(195000, 18), A, A, {
          from: A,
          value: dec(1000, "ether")
        }),
        "Max fee percentage must be between 0.5% and 100%"
      ...
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, "4999999999999999", dec(195000, 18), B, B, {
          from: B,
          value: dec(1200, "ether")
        }),
        "Max fee percentage must be between 0.5% and 100%"
    ...
    it("openTrove(): reverts if fee exceeds max fee percentage", async () => {
      ...
      const lessThan5pct = "49999999999999999";
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, lessThan5pct, dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, "ether")
        }),
        "Fee exceeded provided maximum"
      ...
      // Attempt with maxFee 1%
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, dec(1, 16), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, "ether")
        }),
        "Fee exceeded provided maximum"
      ...
      // Attempt with maxFee 3.754%
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, dec(3754, 13), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, "ether")
        }),
        "Fee exceeded provided maximum"
      ...
      await assertRevert(
        borrowerOperations.openTrove(assetAddress1, 0, dec(5, 15), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, "ether")
        }),
        "Fee exceeded provided maximum"
    ...
    if (!withProxy) {
      ...
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        th.assertIsApproximatelyEqual(
          newDebt,
          D_KUSDRequest.add(emittedFee).add(KUSD_GAS_COMPENSATION),
          100000
        );
      });
    }
    ...
    it("openTrove(): Reverts opening a trove with min debt when system is in Recovery Mode", async () => {
      ...
      await assertRevert(
        borrowerOperations.openTrove(
          ...
          0,
          th._100pct,
          await getNetBorrowingAmount(MIN_NET_DEBT, assetAddress1),
          carol,
          carol,
          { from: carol, value: dec(1, "ether") }
        )
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const expectedDebt = KUSDRequest.add(
        await troveManager.getBorrowingFee(assetAddress1, KUSDRequest)
    ...
    it("openTrove(): adds Trove owner to TroveOwners array", async () => {
      const TroveOwnersCount_Before = (
        await troveManager.getTroveOwnersCount(assetAddress1)
      ).toString();
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_KUSDDebt", async () => {
      ...
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(assetAddress1, bob);
    ...
    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      ...
      // Check Trove is active
      const alice_Trove_1 = await troveManager.Troves(assetAddress1, alice);
      ...
      const alice_Trove_2 = await troveManager.Troves(assetAddress1, alice);
      ...
      const alice_Trove_3 = await troveManager.Troves(assetAddress1, alice);
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      th.assertIsApproximatelyEqual(
        res3,
        KUSD_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12)))
      );
    });
    ...
    if (!withProxy) {
      ...
      it.skip("closeTrove(): fails if owner cannot receive ETH", async () => {
        ...
        const openTroveData = th.getTransactionData(
          ...
          [assetAddress1, _10000Ether, _100pctHex, _1e25Hex, "0x0", "0x0"]
        );

        await nonPayable.forward(borrowerOperations.address, openTroveData);
        assert.equal(
          (await troveManager.getTroveStatus(assetAddress1, nonPayable.address)).toString(),
          "1",
          "NonPayable proxy should have a trove"
        );
        assert.isFalse(
          await th.checkRecoveryMode(contracts, assetAddress1),
