...
contract(
  "Access Control: Kumo functions with the caller restricted to Kumo contract(s)",
  async accounts => {
    ...
    describe("BorrowerOperations", async accounts => {
      it("moveAssetGainToTrove(): reverts when called by an account that is not StabilityPool", async () => {
        ...
        try {
          const tx1 = await borrowerOperations.moveAssetGainToTrove(
            erc20Asset1.address,
            0,
            bob,
            bob,
    ...
    describe("TroveManager", async accounts => {
      ...
      it("applyPendingRewards(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.applyPendingRewards(erc20Asset1.address, bob, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("updateRewardSnapshots(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.updateTroveRewardSnapshots(erc20Asset1.address, bob, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("removeStake(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.removeStake(erc20Asset1.address, bob, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          // assert.include(err.message, "Caller is not the BorrowerOperations contract")
        }
      ...
      it("updateStakeAndTotalStakes(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.updateStakeAndTotalStakes(erc20Asset1.address, bob, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("closeTrove(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.closeTrove(erc20Asset1.address, bob, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          // assert.include(err.message, "Caller is not the BorrowerOperations contract")
        }
      ...
      it("addTroveOwnerToArray(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.addTroveOwnerToArray(erc20Asset1.address, bob, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("setTroveStatus(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.setTroveStatus(erc20Asset1.address, bob, 1, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("increaseTroveColl(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.increaseTroveColl(erc20Asset1.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("decreaseTroveColl(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.decreaseTroveColl(erc20Asset1.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("increaseTroveDebt(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.increaseTroveDebt(erc20Asset1.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("decreaseTroveDebt(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        try {
          const txAlice = await troveManager.decreaseTroveDebt(erc20Asset1.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("ActivePool", async accounts => {
      ...
      it("sendETH(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.sendAsset(erc20Asset1.address, alice, 100, {
            from: alice
          });
        } catch (err) {
      ...
      it("increaseKUSDDebt(): reverts when called by an account that is not BO nor TroveM", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.increaseKUSDDebt(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("decreaseKUSDDebt(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.decreaseKUSDDebt(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("DefaultPool", async accounts => {
      ...
      it("sendETHToActivePool(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.sendAssetToActivePool(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("increaseKUSDDebt(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.increaseKUSDDebt(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("decreaseKUSD(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.decreaseKUSDDebt(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("StabilityPool", async accounts => {
      ...
      it("fallback(): reverts when called by an account that is not the Active Pool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await stabilityPool.receivedERC20(erc20Asset1.address, 100, {
            from: alice
          });
        } catch (err) {
          console.log(err.message);
    ...
    describe("KUSDToken", async accounts => {
      ...
      it("mint(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        const txAlice = kusdToken.mint(erc20Asset1.address, bob, 100, { from: alice });
      ...
      it("returnFromPool(): reverts when called by an account that is not TroveManager nor StabilityPool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await kusdToken.returnFromPool(activePool.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("SortedTroves", async accounts => {
      ...
      it("remove(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await sortedTroves.remove(erc20Asset1.address, bob, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          assert.include(err.message, " Caller is not the TroveManager");
        }
